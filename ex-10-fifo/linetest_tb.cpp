...
void	usage(void) {
	fprintf(stderr, "USAGE: linetest_tb [<filename.txt>]\n");
	fprintf(stderr, "\n"
"\tWhere ... \n"
"\n"
...
int	main(int argc, char **argv) {
	...
	if (child_pid) {
		...
		tb = new TESTB<Vlinetest>;

		// Create an output trace file
		tb->opentrace("linetest.vcd");
		...
		//
		const	unsigned LARGE_NUMBER = (baudclocks * 10 * flen * 2);
		// LARGE_NUMBER should nominally be long enough to encompass
		// the entire length of the simulation, and possibly some more.
		while(testcount++ < LARGE_NUMBER) {
			...
			// i_uart_tx value.  We'll return the output to
			// the parent process via the UARTSIM.
