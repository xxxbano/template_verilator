...
int	UARTSIM::operator()(const int i_tx) {
	...
	if (m_tx_state == TXIDLE) {
		...
		if (!m_tx_busy) {
			m_tx_state = TXIDLE;
			if ((rand() & 0x1f) > 12)
				m_delay = (rand() & 0x07f) * m_baud_counts;
		} else {
			m_tx_data >>= 1;
			m_tx_data |= -1024;
			m_tx_busy >>= 1;
...
// fprintf(stderr, "O_RX = %d, DATA=%03x, BUSY=%03x, CTR = %d, BAUD=%d\n", o_rx, m_tx_data, m_tx_busy, m_tx_baudcounter, m_baud_counts);
	return o_rx;
}
